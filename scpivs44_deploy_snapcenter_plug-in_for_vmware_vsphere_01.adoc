...
. Nella schermata VMware, fare clic su *vSphere Web Client (HTML5)*, quindi accedere alla pagina *VMware vCenter Single Sign-on*.
. Fare clic con il pulsante destro del mouse su qualsiasi oggetto di inventario che sia un oggetto padre valido di una macchina virtuale, ad esempio un data center, una cartella, un cluster o un host, quindi selezionare *Deploy OVF Template* (implementa modello OVF) per avviare la procedura guidata.
